...
const allTools: Tool[] = Object.values(modules).flatMap(m => m.default || []).reduce<RawTool[]>((acc, tool) => { const index = acc.findIndex(x => x.id === tool.id); if (index < 0) return [...acc, tool]; const current = acc[index]; const merged = { ...current } as Record<string, unknown>; Object.entries(tool).forEach(([k, v]) => { if (v !== '' && v !== undefined && v !== null) merged[k] = v }); return acc.map((x,i) => i === index ? merged as RawTool : x) }, []).map(tool => {
  ...
  return { ...tool, company: host ? title(host) : 'Independent', pricingLabel, rating: tool.rating ?? 4.2, openSource: tool.openSource ?? false, apiAvailable: tool.apiAvailable ?? Boolean(tool.website), playstore: tool.playstore || '', description: `${title(tool.category)} tool for modern workflows.` }
      ...
      <AnimatePresence>
        {command&&<motion.div className="command-overlay" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setCommand(false)}>
          <motion.div className="command-box" initial={{scale:.98,y:-8}} animate={{scale:1,y:0}} onClick={e=>e.stopPropagation()}>
            <div className="command-input"><Search size={18}/><input autoFocus placeholder="Search the directory" onChange={e=>{const v=e.target.value; if(v) navigate(`/browse?q=${encodeURIComponent(v)}`)}}/><kbd>ESC</kbd></div>
