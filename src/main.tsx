      ...
      <AnimatePresence>
        {command&&<motion.div className="command-overlay" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setCommand(false)}>
          <motion.div className="command-box" initial={{scale:.98,y:-8}} animate={{scale:1,y:0}} onClick={e=>e.stopPropagation()}>
            <div className="command-input"><Search size={18}/><input autoFocus placeholder="Type to search 100+ AI tools, categories, or updates..." onChange={e=>{const v=e.target.value; if(v) navigate(`/browse?q=${encodeURIComponent(v)}`)}}/><kbd>ESC</kbd></div>
            ...
            <button className="command-intro-btn" onClick={() => { setCommand(false); setShowIntro(true); }}><Sparkles size={16}/> Replay welcome intro</button>
          </motion.div>
        </motion.div>}
      </AnimatePresence>
    </div>
